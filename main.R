...
create_directories(plots_dir, eda_dir, chains_dir, graphs_dir, summaries_dir)
...
discovery_results = ggs_discovery(
  ...
  alpha = 0.1,
  m = 25,
  # Number of imputations (reduced for faster execution)
  maxit = 25,
  # Maximum number of iteration for MICE (reduced for faster execution)
  n.core = parallel::detectCores(),
  plot.graph = TRUE,
  save.graph = TRUE,
  plot.chains = TRUE,
  # Set to TRUE if you want to see convergence plots
  save.chains = TRUE,
  neighborhood = c(1, 2),
  # Save both 1-hop and 2-hop neighborhood graphs
  mincor = 0.1,
